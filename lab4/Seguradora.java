...
public class Seguradora {
    ...
    public boolean cadastrarCliente(Cliente cliente){
        ...
        if(cliente instanceof Cliente_PF){
            ...
            if(Validacao.validarCPF(((Cliente_PF)(cliente)).getCpf()) == true){
            ...
            if(Validacao.validarCNPJ(((Cliente_PJ)cliente).getCnpj()) == true){
                listaClientes.add(cliente);
    ...
    public boolean removerCliente( String documento){
        ...
        for(Cliente cliente: listaClientes){
            if(((Cliente_PF)cliente).getCpf().equals(documento) == true || ((Cliente_PJ)cliente).getCnpj().equals(documento) == true){
    ...
    public int qnt_Sinistros_Cliente(Cliente cliente){
        int contador=0;
    ...
    public double calcularPrecoSeguroCliente(Cliente cliente){
        ...
        double valor = cliente.calculaScore() * (1+qnt_Sinistros_Cliente(cliente));
    ...
    public double calcularReceita(){
        ...
        for(Cliente cliente : listaClientes){
            receita += cliente.calculaScore() * (1+qnt_Sinistros_Cliente(cliente));
    ...
    public void listarSinistros_Cliente(Cliente cliente){
        ...
        for(Sinistro sinistro : listaSinistros){
            ...
            if (sinistro.getCliente() == cliente){
                System.out.println("["+i+"]"+sinistro.toString());
            }
        }

    }

    public Cliente acha_Cliente (String documento){

        for(Cliente cliente : listaClientes){
            if (cliente instanceof Cliente_PF){
                if(((Cliente_PF)cliente).getCpf() == documento){
                    return cliente;
                }

            }
            else if(cliente instanceof Cliente_PJ){
                if(((Cliente_PJ)cliente).getCnpj() == documento){
                    return cliente;
                }

            }
            else{
                System.out.println("Não foi possivel encontrar um cliente com este documento.\nTente novamente.");
    ...
    public void excluirVeiculo(String placa){
        ...
        for(Veiculo veiculo : lista_Geral_de_Veiculos_seguradora){
            if(veiculo.getPlaca() == placa){
                lista_Geral_de_Veiculos_seguradora.remove(veiculo);
            }
        }
    ...
    public void transfereVeiculo(Cliente cliente_remetente, Cliente cliente_destinatario){

        for(Veiculo veiculo : cliente_remetente.getListaVeiculos()){

            cliente_destinatario.getListaVeiculos().add(veiculo);
            cliente_remetente.getListaVeiculos().remove(veiculo);

        }
        calcularPrecoSeguroCliente(cliente_destinatario);
        calcularPrecoSeguroCliente(cliente_remetente);
    }
